...
const ViewPrinters = () => {
  ...
  const { data: printers, isLoading, error } = useQuery<Printer[]>({
    queryKey: ['printers'],
    queryFn: fetchPrinters
  });

  const deleteMutation = useMutation({mutationFn:deletePrinter,
    onSuccess: () => {
    ...
    <div>
      ...
      <ul className="list-disc">
        {printers?.map((printer) => (
          <li key={printer.printerId} className="my-2">
            <span className="font-semibold">{printer.name}</span> 
            - Location: {printer?.location?.address}, {printer?.location?.city}
            <button 
